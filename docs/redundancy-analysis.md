# ALTO Protocol Redundancy Analysis

> Analysis of ALTO state machine vs Claude Code native capabilities (post Feature-6)
...
## What ALTO Adds That's NOT Redundant
...
| ALTO Component | Claude Native Equivalent |
|---------------|--------------------------|
| `alto-planner` "PLANNING" phase | `EnterPlanMode` + `TodoWrite` |
| Role agents (`alto-backend`, `alto-frontend`) | `Task` tool with subagent_type already does this |
| `alto-qa` running tests | Claude can just run tests directly |
| Task status tracking | `TodoWrite` does this |
| `alto-recorder` (within session) | Conversation context already preserved |
...
1. **Replace `alto-planner` with `EnterPlanMode`** - Claude's native planning is sufficient
2. **Merge role agents into Task prompts** - Instead of `alto-backend.md`, just use Task with a backend prompt
3. **Use TodoWrite for in-session tracking** - Only use task files for cross-session persistence
4. **Remove `alto-enforcer`** - Claude Code already has sandboxing and permission controls
5. **Simplify `alto-recorder`** - Within a session, conversation context suffices; only write handoffs at session end

## Core Value Proposition

The ALTO protocol's **irreplaceable value** is:
...
Could Use Native:
  ...
  Task subagents     → instead of separate alto-* agent files
```
...
| `alto-gitops` | | ✓ | | Useful but could be simpler |
| `alto-planner` | | | ✓ | Use EnterPlanMode |
| `alto-enforcer` | | | ✓ | Claude has native safeguards |
| `alto-recorder` | | ✓ | | Only at session boundaries |
| `alto-reviewer` | | ✓ | | Inline with task execution |
